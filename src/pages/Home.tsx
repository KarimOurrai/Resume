...
const Home = () => {
  ...
  return (
    <div
      className={`min-h-screen flex flex-col lg:flex-row ${theme === "light" ? "bg-gray-100" : "bg-[#1a1e23]"}`}
    >
      {/* Left/Right sidebar with social links */}
      <div
        className={`fixed ${isRTL ? "right-0" : "left-0"} h-16 w-screen lg:h-screen lg:w-16 flex flex-row lg:flex-col items-center justify-center gap-6 border-b lg:border-r border-pink-500/20 bg-[#1a1e23]/50 lg:bg-transparent backdrop-blur-sm lg:backdrop-blur-none ${theme === "light" ? "border-pink-500/10" : "border-pink-500/20"} z-10`}
      >
        <a
      ...
      {/* Main content */}
      <div
        className={`flex-1 ${isRTL ? "lg:pr-24" : "lg:pl-24"} pt-36 sm:pt-48 lg:pt-32 px-4 lg:px-8`}
      >
        <div className={`max-w-4xl ${isRTL ? "mr-auto lg:ml-[500px]" : "ml-0 lg:mr-[500px]"}`}>
          <h1
            className={`text-2xl lg:text-3xl mb-4 ${theme === "light" ? "text-gray-900" : "text-white"}`}
          >
            {t("home.greeting")}
            <br />
            <AnimatedName text={t("home.name")} className='font-semibold' />
          </h1>
          <div className='flex items-center gap-2 mb-6'>
            <span
              className={`text-lg lg:text-xl ${theme === "light" ? "text-gray-900" : "text-white"}`}
            >
              {t("home.roles.techLead")}
            </span>
            <span className='text-pink-500'>•</span>
            <span
              className={`text-lg lg:text-xl ${theme === "light" ? "text-gray-900" : "text-white"}`}
            >
              {t("home.roles.softwareEngineer")}
            </span>
          </div>
          <p
            className={`text-base lg:text-lg max-w-2xl mb-8 ${theme === "light" ? "text-gray-600" : "text-gray-400"}`}
          >
            {t("home.description")}
          </p>
          <button
